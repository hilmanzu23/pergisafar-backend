...
export default {
  ...
  methods: {
    ...
    async getData() {
      const response = await MasterService.getData('Status')
    ...
    async postData() {
      ...
      const res = await MasterService.postData('Status', payload)
      if (res.code !== 200) {
        this.dialog = false
        this.isEdit = false
      } else {
    ...
    async updateData() {
      ...
      const res = await MasterService.putData('Status', payload)
      if (res.code !== 200) {
        Swal.fire({
          title: 'Error!',
    ...
    async deleteData(item) {
      Swal.showLoading()
      this.dialog = false
      const res = await MasterService.deleteData('Status', item)
      if (res.code !== 200) {
        Swal.fire({
          title: 'Error!',
          text: res.errorMessage.Error,
