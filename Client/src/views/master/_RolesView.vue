...
export default {
  ...
  methods: {
    ...
    async getData() {
      const response = await MasterService.getData('Role')
    ...
    async postData() {
      ...
      const res = await MasterService.postData('Role', payload)
      if (res.code !== 200) {
        this.dialog = false
        this.isEdit = false
      } else {
    ...
    async updateData() {
      ...
      const res = await MasterService.putData('Role', payload)
      if (res.code !== 200) {
        Swal.fire({
          title: 'Error!',
    ...
    async deleteData(item) {
      Swal.showLoading()
      this.dialog = false
      const res = await MasterService.deleteData('Role', item)
      if (res.code !== 200) {
        Swal.fire({
          title: 'Error!',
          text: res.errorMessage.Error,
