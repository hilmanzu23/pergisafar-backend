...
export default {
  data() {
    return {
      ...
      searchValue: "",
      headers: [
        { text: "Key", value: "key", sortable: true, width: "50%" },
  ...
  methods: {
    ...
    async getData() {
      const response = await MasterService.getData('Setting')
    ...
    async postData() {
      ...
      const res = await MasterService.postData('Setting', payload)
      if (res.code !== 200) {
        this.dialog = false
        this.isEdit = false
      } else {
    ...
    async updateData() {
      ...
      const res = await MasterService.putData('Setting', payload)
      if (res.code !== 200) {
        Swal.fire({
          title: 'Error!',
    ...
    async deleteData(item) {
      Swal.showLoading()
      this.dialog = false
      const res = await MasterService.deleteData('Setting', item)
      if (res.code !== 200) {
        Swal.fire({
          title: 'Error!',
          text: res.errorMessage.Error,
